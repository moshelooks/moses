/****
   STL-like templated tree class, based on tree.hh version 2.3
   (see http://www.aei.mpg.de/~peekas/tree/),  which is
   copyright (C) 2001-2006  Kasper Peeters <kasper.peeters@aei.mpg.de>.
   Used with permission for use in MOSES only.
****/
...
class tree {
   ...
   public:
      ...
      /// Inserts n empty nodes as last children of node pointed to by position.
      ...
      /// Insert n copies of node as last children of node pointed to by position.
      ...
      //// Insert node above position (below parent); returns new node
    ...
    template<class StrictWeakOrdering>
    void     sort_tree_nodes(sibling_iterator from, sibling_iterator to, StrictWeakOrdering comp, bool deep=false);
      ...
      // the only difference between swap_siblings and reorder_siblings
      // is that in the formwer the iterators (passed by reference) are also swapped
      void     swap_siblings(sibling_iterator& a,sibling_iterator& b) { 
        swap_siblings_(a,b); 
        std::swap(a,b);
      }
      void     reorder_siblings(const sibling_iterator& a,const sibling_iterator& b) { 
        swap_siblings_(a,b);
      }
      ...
      /// Compute the length of the long strictly descending path to a leaf
      int      subtree_depth(const iterator_base&) const;
   ...
   private:
      ...
      void     swap_siblings_(const sibling_iterator&,const sibling_iterator&);
};
...
(sibling_iterator from, sibling_iterator to,Partition pred) {
  while (true) {
    ...
    if (from==to)
      return from;
    swap_siblings(to,from);
  }
}

template <class T, class tree_node_allocator>
template <class StrictWeakOrdering>
void tree<T, tree_node_allocator>::sort_tree_nodes(sibling_iterator from, sibling_iterator to, 
						   StrictWeakOrdering comp, bool deep) {
  if(from==to) return;
  // make list of sorted nodes
  // CHECK: if multiset stores equivalent nodes in the order in which they
  // are inserted, then this routine should be called 'stable_sort'.
  std::multiset<tree_node *, StrictWeakOrdering > nodes(comp);
  sibling_iterator it=from, it2=to;
  while(it != to) {
    nodes.insert(it.node);
    ++it;
  ...
  tree_node *prev=from.node->prev_sibling;
  tree_node *next=it2.node->next_sibling;
  typename std::multiset<tree_node *, StrictWeakOrdering >::iterator nit=nodes.begin(), eit=nodes.end();
  if(prev==0) {
    if((*nit)->parent!=0) // to catch "sorting the head" situations, when there is no parent
      (*nit)->parent->first_child=(*nit);
  }
  ...
  if(deep) {	// sort the children of each node too
    ...
    ++ecs;
    while(bcs!=ecs) {
      sort_tree_nodes(begin(bcs), end(bcs), comp, deep);
      ++bcs;
    }
  }
}
...
template <class T, class tree_node_allocator>
int tree<T, tree_node_allocator>::subtree_depth(const iterator_base& it) const {
  int d=-1;
  for (sibling_iterator sib=it.begin();sib!=it.end();++sib)
    d=max(d,subtree_depth(sib));
  return d+1;
}
...
template <class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::swap_siblings_(const sibling_iterator& a,const sibling_iterator& b) {
  //cout << "swapping " << tree<T>(a) << " " << tree<T>(b) << endl;
  tree_node *first=a.node;
  tree_node *second=b.node;
  assert(first->parent==second->parent);
  if (first==second)
    return;
  
  std::swap(first->prev_sibling,second->prev_sibling);
  if (first->prev_sibling)
    first->prev_sibling->next_sibling=first;
  if (second->prev_sibling)
    second->prev_sibling->next_sibling=second;
  std::swap(first->next_sibling,second->next_sibling);
  ...
  if (first->parent) {
    if (first->parent->first_child==first)
      first->parent->first_child=second;
    else if (first->parent->first_child==second)
      first->parent->first_child=first;
    if (first->parent->last_child==first)
      first->parent->last_child=second;
    else if (first->parent->last_child==second)
      first->parent->last_child=first;
  }
...
bool operator==(const tree<T>& t1,const tree<T>& t2) {
  return (t1.size()==t2.size() && t1.equal(t1.begin(),t1.end(),t2.begin()));
}  
template<typename T>
bool operator!=(const tree<T>& t1,const tree<T>& t2) {
  return (!(t1==t2));
...
template<typename T>
static void out_tree_helper(std::ostream& out,const tree<T>&  tr,
                            typename tree<T>::iterator it) {
  int nChildren=it.number_of_children();
  if (nChildren==1) {
    out << (*it) << "(";
    out_tree_helper(out,tr,tr.begin(it));
    out << ")";
  } else if (nChildren==0) {
    out << (*it);
  } else {
    out << (*it) << "(";
    out_tree_helper(out,tr,tr.begin(it));
    for (typename tree<T>::sibling_iterator sib=++tr.begin(it);
         sib!=tr.end(it);++sib) {
      out << " ";
      out_tree_helper(out,tr,sib);
    }
    out << ")";
  }
}
...
std::ostream& operator<<(std::ostream& out, const tree<T>& tr) {
  //print_tree(tr,out);
  for (typename tree<T>::iterator it=tr.begin();it!=tr.end();++it) {
    out_tree_helper(out,tr,it);
    it.skip_children();
    out << " ";
  }
  return out;
...
template<typename iter>
bool equal_subtree(iter it1,iter it2) {
  ...
  if (it1.number_of_children()!=it2.number_of_children())
    return false;
  for (typename tree<T>::sibling_iterator c1=it1.begin(),c2=it2.begin();
       c1!=it1.end();++c1,++c2)
    if (!equal_subtree(c1,c2))
      return false;
  return true;
}
...
iter move_before(iter target, iter source) {
  typedef tree_node_<typename std::iterator_traits<iter>::value_type> 
    tree_node;
  tree_node *dst=target.node;
  tree_node *src=source.node;
  assert(dst);
...
iter insert_subtree(iter1 position,iter2 subtree) {

}

/// Append nodes in from-to range (+ their children) as children of position.
template<typename iter> 
...
template<typename iter>
int subtree_size(iter it) {
  typedef typename iter::value_type T;
  int sz=1;
  for (typename tree<T>::sibling_iterator child=it.begin();child!=it.end();
       ++child)
